...
describe( 'SlotResolver.compilePlan — the shared core', () => {
	...
	it( 'the more specific source layer (injected) wins over a lens-carried member of the same class', () => {
		const lensMember     = block( { path: 'lens-habit.html', sourceLayer: 'lens', habitClass: 'session-logging', text: 'aggressive' } );
		const injectedMember = block( { path: 'injected-habit.html', sourceLayer: 'injected', habitClass: 'session-logging', text: 'never' } );
		const plan = SlotResolver.compilePlan( [ lensMember, injectedMember ] );

		expect( plan.slots ).toHaveLength( 1 );
		expect( plan.slots[ 0 ].habitClass ).toBe( 'session-logging' );
	...
	it( 'the winner keeps its own original position in load order — a per-position filter, not a reposition', () => {
		...
		const loser  = block( { path: 'lens-habit.html', sourceLayer: 'lens', habitClass: 'session-logging', text: 'aggressive' } );
		const winner = block( { path: 'injected-habit.html', sourceLayer: 'injected', habitClass: 'session-logging', text: 'never' } );
	...
	it( 'two independent habit-classes resolve independently', () => {
		const sessionWinner   = block( { path: 's.html', sourceLayer: 'injected', habitClass: 'session-logging', text: 's' } );
		const completedWinner = block( { path: 'c.html', sourceLayer: 'lens', habitClass: 'completed-logging', text: 'c' } );
	...
	it( 'a single-candidate class still resolves trivially — the lone candidate wins its own slot', () => {
		const lone = block( { path: 'no-subagents.html', sourceLayer: 'lens', habitClass: 'subagent-policy', text: 'no subagents' } );
		const plan = SlotResolver.compilePlan( [ lone ] );
		expect( plan.slots ).toHaveLength( 1 );
		expect( plan.slots[ 0 ].winner.path ).toBe( 'no-subagents.html' );
	...
	it( 'regression: a classed artifact that emits SEVERAL blocks (head + multiple sections) never competes against itself — all its blocks survive together', () => {
		...
		const head   = block( { path: 'habit.html', habitClass: 'session-logging', text: 'head' } );
		const when   = block( { path: 'habit.html', habitClass: 'session-logging', text: 'when' } );
		const action = block( { path: 'habit.html', habitClass: 'session-logging', text: 'action' } );
	...
	it( 'regression: when a multi-block artifact LOSES, every one of its blocks is dropped — not just its head', () => {
		const loserHead   = block( { path: 'loser.html', sourceLayer: 'lens', habitClass: 'session-logging', text: 'loser head' } );
		const loserWhen   = block( { path: 'loser.html', sourceLayer: 'lens', habitClass: 'session-logging', text: 'loser when' } );
		const winner      = block( { path: 'winner.html', sourceLayer: 'injected', habitClass: 'session-logging', text: 'winner text' } );
...
describe( 'SlotResolver.describe / compile — thin reads of the same plan', () => {
	const lensMember     = block( { path: 'lens-habit.html', sourceLayer: 'lens', habitClass: 'session-logging', text: 'aggressive text' } );
	const injectedMember = block( { path: 'injected-habit.html', sourceLayer: 'injected', habitClass: 'session-logging', text: 'never text' } );
